#include "CompleteSceneFadeOutTransitionPrefab.h"
...
ALA_CLASS_SOURCE_1(CompleteSceneFadeOutTransitionPrefab, ala::PrefabV2)

void CompleteSceneFadeOutTransitionPrefab::doInstantiate(ala::GameObject* object, std::istringstream& argsStream) const {
  // args
  // constants
  ...
  new State(stateManager, "default",
    ...
    if (!fadeIn->isRunning()) {
      //chuyen scene
      gameManager->replaceScene(new CompleteScene());
    }
  },
    NULL);
