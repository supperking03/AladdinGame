#include "DieSceneFadeOutTransitionPrefab.h"
...
ALA_CLASS_SOURCE_1(DieSceneFadeOutTransitionPrefab, ala::PrefabV2)

void DieSceneFadeOutTransitionPrefab::doInstantiate(ala::GameObject* object, std::istringstream& argsStream) const {
  // args
  ...
  new State(stateManager, "default",
    ...
    if (!fadeIn->isRunning()) {
      //chuyen scene
      gameManager->replaceScene(new DieScene(mapResource));
    }
  },
    NULL);
